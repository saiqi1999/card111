...
- **动态标题描述**：容器支持动态设置标题和描述，正确显示召唤者卡牌信息
- **召唤者关联**：容器与召唤卡牌建立关联，支持智能交互检测
- **单例管理**：确保场景中只存在一个容器实例，自动管理容器生命周期
- **大容器系统**：支持更大尺寸的容器界面，提供更丰富的交互体验
- **统一层级管理**：使用全局常量管理所有UI元素的z_index，确保层级一致性
