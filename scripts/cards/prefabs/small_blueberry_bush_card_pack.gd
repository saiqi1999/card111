...
func small_blueberry_bush_after_recipe_done(card_instance, crafting_cards: Array):
	...
	if blueberry:
		...
		GlobalUtil.log("小蓝莓丛收割：生成蓝莓，随机移动距离: " + str(move_distance), GlobalUtil.LogLevel.INFO)
	...
	if should_generate_magic:
		...
		if magic_aura:
			...
			GlobalUtil.log("小蓝莓丛收割：生成小魔法气息，随机移动距离: " + str(move_distance), GlobalUtil.LogLevel.INFO)
