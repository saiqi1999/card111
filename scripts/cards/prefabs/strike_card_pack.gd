...
func strike_click_effect(card_instance):
	var random_number = randi() % 100 + 1  # 生成1-100的随机数
	GlobalUtil.log("卡牌实例ID:" + str(card_instance.get_instance_id()) + " 打击卡牌特效触发！随机数: " + str(random_number), GlobalUtil.LogLevel.INFO)
