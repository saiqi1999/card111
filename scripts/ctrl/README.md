...
容器系统用于在游戏中显示和管理各种容器，如背包、商店界面等。采用Control+Anchor的UI实现方式。
...
容器系统采用Control+Anchor的UI实现方式，具有以下特点：

1. **UI原生支持** - 基于Control节点，享受Godot原生UI系统的所有优势
2. **响应式布局** - 使用Anchor系统实现自适应布局
3. **层级管理** - 通过UI层级自然管理显示顺序
4. **易于扩展** - 新增容器类型只需继承基类并设置参数
...
2. **Anchor布局**: 使用Anchor系统实现响应式布局，适应不同屏幕尺寸
3. **单例管理**: 确保场上只能存在一个容器实例，避免UI冲突
4. **自动布局**: 容器自动定位到左下角，无需手动设置位置
5. **动态内容**: 支持动态设置标题和描述，适应不同使用场景
6. **类型系统**: 通过类型字符串管理不同容器类型，易于扩展
7. **生命周期管理**: 完善的创建、初始化和销毁流程
8. **UI层级**: 基于Control的自然层级管理，无需手动设置z_index
